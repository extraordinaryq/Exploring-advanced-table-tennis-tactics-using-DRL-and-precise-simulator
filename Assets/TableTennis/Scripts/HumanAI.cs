...
public class PaddleAi : Agent
{
    ...
    [SerializeField] float boundX = 5f;       // X 方向边界：[-5, 5]
    [SerializeField] float boundZ = 5f;       // Z 方向边界：[-5, 5]
    [SerializeField] bool useLocal = false;  // 用局部坐标还是世界坐标
    [SerializeField] Transform boundsOrigin;  // 边界中心（不设则用(0,0,0)或本地(0,0,0)）
    ...
    bool BallOutOfBoundsXZ()
    {
        if (ball == null) return false;

        Vector3 p = useLocal ? ball.transform.localPosition : ball.transform.position;
        Vector3 ctr = Vector3.zero;
        if (boundsOrigin != null)
            ctr = useLocal ? boundsOrigin.localPosition : boundsOrigin.position;

        float dx = p.x - ctr.x;
        float dz = p.z - ctr.z;
        Debug.Log(dx);
        return (Mathf.Abs(dx) > boundX) || (Mathf.Abs(dz) > boundZ);
    }
}